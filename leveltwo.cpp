...
LevelTwo::LevelTwo(QScrollBar*s , QObject *parent) : QGraphicsScene(0,0,0,0, parent)
  , m_velocity(7)
  , m_skippedMoving(0)
  , m_groundLevel(0)
  , m_minX(0)
...

{
    ...
    connect(&m_timer, &QTimer::timeout, this, &LevelTwo::movePlayer);

    //Timer to controll player fall speed
    mFallTimer.setInterval(20);
    connect(&mFallTimer, &QTimer::timeout, this, &LevelTwo::fallPlayer);
    ...
    connect(this, &LevelTwo::jumpFactorChanged, this, &LevelTwo::jumpPlayer);
    connect(m_jumpAnimation, &QPropertyAnimation::stateChanged, this, &LevelTwo::jumpStatusChanged);
...
void LevelTwo::keyPressEvent(QKeyEvent *event) {
...
void LevelTwo::keyReleaseEvent(QKeyEvent *event) {
    if (event->isAutoRepeat())
        return;
...
void LevelTwo::jumpStatusChanged(QAbstractAnimation::State newState, QAbstractAnimation::State oldState) {
    if(newState == QAbstractAnimation::Stopped && oldState == QAbstractAnimation::Running) {
    }
}

//Make player detect other platform when falling
void LevelTwo::fallPlayer() {
...
void LevelTwo::initPlayField() {
    ...
    //add
    m_Background = new BackgroundItem(QPixmap(":images/L2_2"));
...
void LevelTwo::timerEvent(QTimerEvent *) {

}

qreal LevelTwo::jumpFactor() const{
    return m_jumpFactor;
}

void LevelTwo::setJumpFactor(const qreal &jumpFactor) {
...
QGraphicsItem* LevelTwo::collidingPlatforms() {
...
bool LevelTwo::handleCollisionWithPlatform() {
