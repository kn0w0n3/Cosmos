...
LevelFour::LevelFour(QScrollBar*s , QObject *parent) : QGraphicsScene(0,0,0,0, parent)
  , m_velocity(7)
  , m_skippedMoving(0)
  , m_groundLevel(0)
  , m_minX(0)
...

{
    ...
    connect(&m_timer, &QTimer::timeout, this, &LevelFour::movePlayer);

    //Timer to controll player fall speed
    mFallTimer.setInterval(20);
    connect(&mFallTimer, &QTimer::timeout, this, &LevelFour::fallPlayer);
    ...
    connect(this, &LevelFour::jumpFactorChanged, this, &LevelFour::jumpPlayer);
    connect(m_jumpAnimation, &QPropertyAnimation::stateChanged, this, &LevelFour::jumpStatusChanged);
...
void LevelFour::keyPressEvent(QKeyEvent *event) {
...
void LevelFour::keyReleaseEvent(QKeyEvent *event) {
    if (event->isAutoRepeat())
        return;
...
void LevelFour::jumpStatusChanged(QAbstractAnimation::State newState, QAbstractAnimation::State oldState) {
    if(newState == QAbstractAnimation::Stopped && oldState == QAbstractAnimation::Running) {
    }
}

//Make player detect other platform when falling
void LevelFour::fallPlayer() {
...
void LevelFour::initPlayField() {
    ...
    //add
    m_Background = new BackgroundItem(QPixmap(":images/L4"));
...
void LevelFour::timerEvent(QTimerEvent *) {

}

qreal LevelFour::jumpFactor() const{
    return m_jumpFactor;
}

void LevelFour::setJumpFactor(const qreal &jumpFactor) {
...
QGraphicsItem* LevelFour::collidingPlatforms() {
...
bool LevelFour::handleCollisionWithPlatform() {
