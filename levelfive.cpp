...
LevelFive::LevelFive(QScrollBar*s , QObject *parent) : QGraphicsScene(0,0,0,0, parent)
  , m_velocity(7)
  , m_skippedMoving(0)
  , m_groundLevel(0)
  , m_minX(0)
...

{
    ...
    connect(&m_timer, &QTimer::timeout, this, &LevelFive::movePlayer);

    //Timer to controll player fall speed
    mFallTimer.setInterval(20);
    connect(&mFallTimer, &QTimer::timeout, this, &LevelFive::fallPlayer);
    ...
    connect(this, &LevelFive::jumpFactorChanged, this, &LevelFive::jumpPlayer);
    connect(m_jumpAnimation, &QPropertyAnimation::stateChanged, this, &LevelFive::jumpStatusChanged);
...
void LevelFive::keyPressEvent(QKeyEvent *event) {
...
void LevelFive::keyReleaseEvent(QKeyEvent *event) {
    if (event->isAutoRepeat())
        return;
...
void LevelFive::jumpStatusChanged(QAbstractAnimation::State newState, QAbstractAnimation::State oldState) {
    if(newState == QAbstractAnimation::Stopped && oldState == QAbstractAnimation::Running) {
    }
}

//Make player detect other platform when falling
void LevelFive::fallPlayer() {
...
void LevelFive::initPlayField() {
    ...
    //add
    m_Background = new BackgroundItem(QPixmap(":images/L5"));
...
void LevelFive::timerEvent(QTimerEvent *) {

}

qreal LevelFive::jumpFactor() const{
    return m_jumpFactor;
}

void LevelFive::setJumpFactor(const qreal &jumpFactor) {
...
QGraphicsItem* LevelFive::collidingPlatforms() {
...
bool LevelFive::handleCollisionWithPlatform() {
